...
#load RRHO function
source("code/TransitionMapping.R") 
...
names(test_data) <- gsub(".csv$","", input_files)
...
save(hypermat.all,file = paste0(outputFolder,"hypermatAll_",condition,".rdata"))

#load(file=paste0(outputFolder,"hypermatAll_",condition,".rdata"))
...
for (k in c(1:length(test_data))) {
  ...
  png(paste0(outputFolder,transition,"_brainSpan_RRHOMap.png"),width = 10, height = 10, units = "in", res = 300)
...
png(paste0(outputFolder,"brainSpan_RRHOMap_color_bar.png",sep = ""), width = 2, height = 6, units = "in", res = 300)
